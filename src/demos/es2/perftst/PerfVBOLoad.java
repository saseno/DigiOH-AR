...
public class PerfVBOLoad extends PerfModule {
	...
	protected void runOneSet(GLAutoDrawable drawable, int dataType, int numObjs, int numVertices, int loops,
		...
		System.out.println("");
		System.out.println("Loops " + loops + ", useVBO " + useVBO + ", objects " + numObjs + ", type "
				+ getTypeName(dataType) + ", vertices p.o. " + vertices[0].getElementCount() + ", colors p.o. "
				+ colors[0].getElementCount() + ",\n total elements " + (verticesElements + colorsElements)
				+ ", total bytes " + (verticesBytes + colorsBytes) + ", total time: " + dt + "ms, fps(-1): "
				+ (((loops - 1) * 1000) / dt) + ",\n col.vert./s: "
				+ ((double) (loops * verticesElements) / ((double) dt / 1000.0)));
		...
		for (int i = 0; i < loops; i++) {
			...
			if (dtT <= 0)
				dtT = 1;
			System.out.println("\tloop " + i + ": clear " + dtC + "ms, finish " + dtF + ", swap " + dtS + "ms, total: "
					+ dtT + "ms, fps " + 1000 / dtT);
			/*
			 * for(int j=0; j<numObjs; j++) { dt = t1[i][j] - t0[i]; dt2= t2[i][j] -
			 * t1[i][j]; dt3= t3[i][j] - t2[i][j]; dtT= dt+dt2+dt3;
			 * System.out.println("\t\tobj "+j+": vertices "+dt
			 * +"ms, colors "+dt2+"ms, draw "+dt3+"ms, total: "+ dtT); }
			 */
		}
		System.out.println("*****************************************************************");
