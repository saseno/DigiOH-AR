...
public class HWShadowmapsSimple extends Demo {
	...
	public void init(GLAutoDrawable drawable) {
		...
		/**
		 * GL _gl = drawable.getGL(); // Debug .. _gl = _gl.getContext().setGL(
		 * GLPipelineFactory.create("com.jogamp.opengl.Debug", GL2.class, _gl, null) );
		 * // Trace .. _gl = _gl.getContext().setGL(
		 * GLPipelineFactory.create("com.jogamp.opengl.Trace", GL2.class, _gl, new
		 * Object[] { System.err } ) );
		 */
		...
		try {
			decal = TextureIO.newTexture(
					getClass().getClassLoader().getResourceAsStream("demos/data/images/decal_image.png"), true,
					TextureIO.PNG);
			...
			light_image = TextureIO.newTexture(
					getClass().getClassLoader().getResourceAsStream("demos/data/images/nvlogo_spot.png"), true,
					TextureIO.PNG);
		} catch (IOException e) {
		...
		pbuffer = GLDrawableFactory.getFactory(GLProfile.getDefault()).createOffscreenAutoDrawable(null, caps, null,
				TEX_SIZE, TEX_SIZE);
		...
		viewer.setOrientation(new Rotf(Vec3f.Y_AXIS, (float) Math.toRadians(45.0f))
				.times(new Rotf(Vec3f.X_AXIS, (float) Math.toRadians(-15.0f))));
	...
	class PbufferListener implements GLEventListener {
		...
		public void init(GLAutoDrawable drawable) {
			...
			if (depth_bits[0] == 16)
				depth_format = GL2.GL_DEPTH_COMPONENT16;
			else
				depth_format = GL2.GL_DEPTH_COMPONENT24;

			light_view_depth = genTexture(gl);
			gl.glBindTexture(GL2.GL_TEXTURE_2D, light_view_depth);
			gl.glTexImage2D(GL2.GL_TEXTURE_2D, 0, depth_format, TEX_SIZE, TEX_SIZE, 0, GL2.GL_DEPTH_COMPONENT,
					GL2.GL_UNSIGNED_INT, null);
		...
		public void display(GLAutoDrawable drawable) {
			...
			gl.glPolygonOffset(tweaks.get(POLYGON_OFFSET_SCALE).val, tweaks.get(POLYGON_OFFSET_BIAS).val);
	...
	private static Mat4f frustumInverse(float left, float right, float bottom, float top, float zNear, float zFar) {
		Mat4f m = new Mat4f();
		m.makeIdent();
